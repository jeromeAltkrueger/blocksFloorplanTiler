...
def detect_trim_offset(page: fitz.Page, metadata: Dict) -> Tuple[float, float]:
    """
    ...
    if abs(pretrim_w - img_w) <= 1 and abs(pretrim_h - img_h) <= 1:
        logger.info("No whitespace trimming detected")
        return (0.0, 0.0)

    logger.info(f"Whitespace trimming detected: {pretrim_w:.0f}x{pretrim_h:.0f} -> {img_w}x{img_h}")
    ...
    if not bbox:
        logger.warning("Could not detect content bbox")
        return (0.0, 0.0)
    ...
    logger.info(f"Trim offset: left={trim_left:.1f}, top={trim_top:.1f} px")
    return (trim_left, trim_top)


def annotation_transform_coords(leaflet_coords: List[float], metadata: Dict,
...
def draw_annotation_polygon(page: fitz.Page, coordinates: List, metadata: Dict,
                            config: Dict, trim_offset: Tuple[float, float] = (0.0, 0.0)) -> None:
    """Draw a filled polygon on the PDF page."""
    outer_ring = coordinates[0]
    pdf_points = []
    for point in outer_ring:
        x_pdf, y_pdf = annotation_transform_coords([point[0], point[1]], metadata, trim_offset)
...
def draw_annotation_marker(page: fitz.Page, coordinates: List[float], metadata: Dict,
                           config: Dict, label: str = None, overlay: str = None,
    ...
    x_pdf, y_pdf = annotation_transform_coords([x, y], metadata, trim_offset)
    radius = config["radius"]

    if overlay:
        font_size = min(radius * 0.5, 4)
    ...
    if label:
        ...
        font_size_l = text_config["font_size"]
        padding = text_config["padding"]
        tw = len(label) * font_size_l * 0.6
        rect = fitz.Rect(x_pdf - padding, y_pdf + radius + 5 - padding,
                         x_pdf + tw + padding, y_pdf + radius + 5 + font_size_l + padding)
        shape2 = page.new_shape()
        shape2.draw_rect(rect)
        shape2.finish(fill=text_config["background_color"], fill_opacity=text_config["background_opacity"])
        shape2.commit()
        page.insert_text(fitz.Point(x_pdf, y_pdf + radius + 5 + font_size_l),
                         label, fontsize=font_size_l, color=text_config["font_color"])


def annotate_pdf(pdf_bytes: bytes, objects: List[Dict], metadata: Dict) -> bytes:
    """Annotate a PDF with shapes and markers."""
    doc = fitz.open(stream=pdf_bytes, filetype="pdf")
    page = doc[0]

    logger.info(f"PDF annotation: {page.rect.width:.0f}x{page.rect.height:.0f} pt, {len(objects)} objects")
    ...
    for i, obj in enumerate(objects):
        try:
            ...
            if obj_type in ("rectangle", "square") or geo_type == "Polygon":
                config = ANNOTATION_CONFIG.get("square" if obj_type == "square" else "polygon")
                draw_annotation_polygon(page, coordinates, metadata, config, trim_offset)
                objects_drawn += 1
                ...
                draw_annotation_marker(page, coordinates, metadata, config, label, overlay, trim_offset)
                objects_drawn += 1
            else:
                logger.warning(f"Unknown annotation type: {obj_type}")
        except Exception as e:
            logger.error(f"Error drawing object {i + 1}: {str(e)}", exc_info=True)
            continue

    logger.info(f"Annotation complete: {objects_drawn}/{len(objects)} objects drawn")
...
async def pdf_annotation_endpoint(request: PdfAnnotationRequest,
    ...
    try:
        ...
        annotated_pdf_bytes = annotate_pdf(pdf_bytes, request.objects, metadata)
